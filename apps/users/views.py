...
from organization.models import CourseOrg,team
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(request, "login.html", {"msg": "账号未激活，请到您的邮箱激活您的账号！"})
            else:
...
class RegisterView(View):
    ...
    def post(self,request):
        ...
        if register_form.is_valid():
            ...
            user_message.message="欢迎注册天釜共享在线网"
            user_message.save()

            send_register_email(user_name,"register")
            return render(request, "login.html",{"msg":"请到您的邮箱激活您的账号并登录"})
        else:
...
class MyFavTeamView(View):
    def get(self,request):
        team_list=[]
        fav_teams = UserFavorite.objects.filter(user=request.user, fav_type=3)
        for fav_team in fav_teams:
            team_id = fav_team.fav_id
            teamm = team.objects.get(id=team_id)
            team_list.append(teamm)
...
class IndexView(View):
    def get(self,request):
        ...
        courses= Course.objects.filter(is_banner=True)[:2]
